...
class Encoder(nn.Module):
    def __init__(self,
                 ...
                 PADDING_TOKEN):
        super().__init__()
        self.sentence_embedding = SentenceEmbedding(seq_length, d_model, lang_to_ind, START_TOKEN, END_TOKEN, PADDING_TOKEN)
...
class MultiheadCrossAttention(nn.Module):
    def __init__(self,d_model,num_heads):
        ...
        self.d_model = d_model
        self.num_heads = num_heads
        assert d_model%num_heads==0, "Incorrect values"
        self.head_dim = d_model//num_heads
    ...
    def forward(self,x,y,mask):
        ...
        values,attention = scaled_dot_product(q,k,v,mask)
        values = values.permute(0,2,1,3).reshape(batch_size,seq_length,d_model)
...
class DecoderLayer(nn.Module):
    def __init__(self,d_model,ffn_hidden,num_heads,drop_prob):
        ...
        self.encode_decoder_attention = MultiheadCrossAttention(d_model=d_model,num_heads=num_heads)
        self.layer_norm2 = LayerNormalization(parameters_shape=[d_model])
        self.dropout2 = nn.Dropout(p=drop_prob)

        self.ffn = PositionFeedForward(d_model=d_model,hidden=ffn_hidden,drop_prob=drop_prob)
        self.layer_norm3 = LayerNormalization(param_shape=[d_model])
    ...
    def forward(self,x,y,self_attention_mask,cross_attention_mask):
        ...
        _y = y.clone()
        y = self.encode_decoder_attention(x,y,mask=cross_attention_mask)
        y = self.dropout2(y)
